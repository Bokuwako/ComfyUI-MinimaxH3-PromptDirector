...
| 노드 | 하는 일 |
|---|---|
| 🎬 **MiniMax H3 Prompt Writer (Ollama)** | 메인. 자연어 → (선택: 연출 확장) → 규격 프롬프트 |
| 🎬 **MiniMax H3 Prompt Validator** | 모델 호출 없이 프롬프트 규격만 검사·자동수리 (오프라인) |
| 🎬 **MiniMax H3 Image Describe (Ollama)** | VLM이 레퍼런스 이미지를 실제로 뭐라고 읽는지 확인용 (디버깅) |
...
### 3.45 Enhancer — 짧은 요청을 연출 브리프로 먼저 부풀리기

`enhance = True` 로 켜면 **2단계**로 돕니다.

```
짧은 자연어  ──[1단계 Enhancer]──>  연출 브리프  ──[2단계 Writer]──>  H3 규격 프롬프트
 "자전거 타고 내려간다"        (입력과 같은 언어)              (영어, 규격)
```

1단계는 **H3 규격을 전혀 모릅니다.** 오직 연출만 채웁니다. 짧은 아이디어에서 거의 항상
빠지는 것들을 보충합니다:

1. **시간** — 0초에 화면에 뭐가 있나(없는 것 포함), 언제 바뀌나, 각 상태가 얼마나 가나
2. **프레임 경계** — 화면 밖에서 들어와 화면 밖으로 나간다, 무엇이 프레임에 안 보이나
3. **시간에 따른 빛** — 주광원 방향과 그것이 비트마다 어떻게 변하나
4. **절제** — "고개를 아주 조금", "손가락만 움직인다" (모델은 항상 과하게 합니다)
5. **가드레일** — 카메라는 회전하지 않는다, 컷하지 않는다, 새 인물이 등장하지 않는다
6. **자세 → 움직임 순서** (아래)
7. **발명 금지** (아래)

#### 규칙 6 — 자세를 먼저, 움직임은 나중

동작의 **이름은 유지합니다**(의미 앵커). 다만 이름만 쓰고 끝내지 않고 이 순서로 풉니다:

| 단계 | 내용 |
|---|---|
| **a. 자세** | 각자가 어느 쪽을 향하는지, 몸의 축(수직·수평·기울기), 지지점과 접촉점, 사지 배치 |
| **b. 공간 관계** | 참여자가 둘 이상이면 누가 위/뒤/앞/옆인지, 거리, 무엇이 무엇에 닿았는지 |
| **c. 움직임** | 누가 주도하는지, 방향, 진폭, 템포 |
| **d. 종속 운동** | c 때문에 **따라 움직이는 것** — 머리카락, 옷감, 호흡, 대상물, 상대의 반응 |

**비대칭 강제.** 참여자들의 역할이 다르면 **한 문장에 묶지 않습니다.** 각자 자기 절과
자기 자세를 갖습니다. 문장을 공유하면 자세도 공유돼서, 모델이 "평균적인 참여자"를
같은 자세로 두 번 렌더링합니다. 두 명 이상이 나오는 동작에서 가장 흔한 실패고,
모델이 아니라 **글쓰기가 원인**입니다.

자세는 동작 이름에서 **도출**되는 것이지 취향으로 지어내는 게 아닙니다.

#### 규칙 7 — 요청하지 않은 것은 발명하지 않는다

브리프가 지정한 것은 끝까지 구체화하되, 지정하지 않은 것은 **중립적으로 남깁니다.**
특히 속도·강도·태도 부사(`steadily`, `slowly`, `gently`, `vigorously`)를 임의로 붙이지
않습니다. 요청하지 않은 부사 하나가 님이 하지 않은 선택을 프롬프트에 확정시켜 버립니다.

> 검사법: `enhanced_brief` 에서 부사를 하나씩 짚어보고 "내가 이걸 말했나?" 물어보세요.
> 아니면 빠져야 할 놈입니다.

**Director의 이미지와 모드를 그대로 재사용합니다.** 이미지를 따로 연결할 필요 없고,
REF2VA면 `picture 1의 여자 캐릭터`, `picture 5의 지하철 내부` 식으로 자동 바인딩합니다.

**출력 언어는 입력 언어를 따라갑니다.** 한국어로 쓰면 한국어 브리프가 나와서 읽고 고칠 수 있습니다.

| 위젯 | 설명 |
|---|---|
| `enhance` | 확장 단계 on/off (기본 OFF) |
| `genre` | 연출 문법 프리셋 11종 (아래) |
| `enhance_override` | 확장 결과를 손보고 여기 붙여넣으면 확장을 건너뛰고 이걸 씀 |

출력 `enhanced_brief` 를 `PreviewAny` 에 물려서 확인하고, 마음에 안 들면 고쳐서
`enhance_override` 에 붙여넣는 게 기본 작업 루프입니다.

**장르 프리셋 11종** (`mmh3/genres.json`)

| # | 장르 | 특징 |
|---|---|---|
| 00 | Auto | 자연어에서 장르 판별 |
| 01 | 일상·슬라이스 오브 라이프 | 고정 카메라, 긴 호흡, 작은 동작 |
| 02 | 액션 | 트래킹·핸드헬드, 매 비트 상승 |
| 03 | POV·1인칭 | 눈높이 고정, 손·팔만 보임, 회전 금지 |
| 04 | 이동·여정 | 문턱(터널·문·능선) 기준 3비트, 배경만 흐름 |
| 05 | 감정·로맨스 | 느린 푸시인, 작은 신체 신호 하나 |
| 06 | 호러·서스펜스 | 오래 참기, 프레임 가장자리, 침묵 |
| 07 | 광고·제품 | 정밀한 기계적 무빙, 히어로 홀드 |
| 08 | 뮤직비디오 | 박자에 맞춘 컷·무빙 |
| 09 | 성인·친밀 장면 | 카메라 관계 고정, 컷 없음, 지속·절제 중심 |
| 10 | 자유 | 문법 강제 없이 브리프만 확장 |

장르는 **그림체가 아니라 연출 축**입니다(카메라·박자·비트 형태·조명 변화·동선·가드레일).
그림체는 `style` 프리셋이 계속 담당합니다. 둘은 같이 씁니다.

> 비용: 확장을 켜면 Ollama 호출이 1회 더 늘어납니다. 이미지가 있으면 그 호출도 비전 호출입니다.
...
역할은 **Enhancer에도 전달**되므로 확장 브리프의 `picture N` 바인딩도 정확해집니다.
...
| `enhanced_brief` | 1단계 확장 결과. `enhance` 가 꺼져 있으면 빈 문자열 |
